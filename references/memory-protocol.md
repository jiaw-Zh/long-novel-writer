...
2. 当前 `volumes/volume-NNN.md`（L4）+ 当前 `arcs/arc-NNN.md`（L3）
   - **前期过渡（按实际存在的最高层加载）**：`layered_summary` 槽位按以下规则拼接存在的段，缺失段跳过：
     | 状态 | layered_summary 填充 |
     |---|---|
     | L5/L4/L3 全部缺失（通常第 1 卷前期）| `story-bible.md` 全文 |
     | 只有 L5 存在（极少见）| L5 |
     | L5+L4 存在，L3 缺失（新卷前期）| L5 + L4-当前卷 |
     | L5+L4+L3 全部存在（稳态）| L5 + L4 + L3 三段拼接 |
     | 只有 L4 存在，无 L5（第 1 卷结束但 L5 未重生成）| L4-当前卷 |
...
4. **追加 `canon/facts.jsonl`**：ID 用 `lnw next-id fact`，本章新增约束性事实（含 `known_by`）
5. **追加 `canon/promises.jsonl`**：ID 用 `lnw next-id promise`
6. **追加 `canon/progression.jsonl`**：ID 用 `lnw next-id prog`
7. **追加 `canon/timeline.md`**：本章时间线事件
8. **追加/更新 `canon/rules.md`**：本章确立或违反的世界规则
9. 追加实体档案的「关键节点」与「变更记录」（引用 FACT ID）；更新「当前状态」中的「最后出场」与「已知秘密」
10. 更新 `foreshadowing-ledger.md` 状态；更新 `subplots.md` 中本章推进的副线
11. 若当前章节是 chunk/arc/volume 的末章，生成对应 L2/L3/L4
