...
**`canon/facts.jsonl`** — 原子事实，每行一条断言，只追加不修改：

```jsonl
{"id":"FACT-0001","chapter":3,"entity":["zhang-san"],"fact":"张三天赋为火系，无法修炼水系功法","source":"测灵石发出赤红光芒"}
```

写后校验时，按本章 `index.md` 的 `entity` 列表过滤相关条目，逐条比对本章内容是否违反。发现错误时追加一条 `type: retraction` 条目并在 `continuity-issues.md` 登记，不删除原条目。
...
2. **硬约束检查**：出场实体的行为是否违背其「硬约束」段？
3. **桥段重复检查**：本章 `plot_beats` 是否与最近 20 章的 `chapter-*.index.md`→`plot_beats` 重复？
4. **伏笔账本对齐**：本章埋设/回收的伏笔是否都能在 `foreshadowing-ledger.md` 找到对应 ID？
...
4. **追加 `canon/facts.jsonl`**：本章新增的约束性事实
5. **追加 `canon/timeline.md`**：本章时间线事件
6. **追加/更新 `canon/rules.md`**：本章确立或违反的世界规则
7. 追加实体档案的「关键节点」与「变更记录」
8. 更新实体档案的「当前状态」
9. 更新 `foreshadowing-ledger.md` 状态
10. 若当前章节是 chunk/arc/volume 的末章，生成对应 L2/L3/L4

canon 账本（步骤 4-6）必须在实体档案（步骤 7-8）之前落盘，确保实体档案的变更记录可以引用 FACT ID。
