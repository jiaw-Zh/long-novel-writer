...
**有 shell 权限时**：直接运行 `lnw assemble-context <N>`，输出即为完整 9 段上下文，无需手工操作。

**无 shell 权限时**：按以下 9 步手工加载：
...
- **阶段 B（校验）**：用阶段 A 的草稿作为输入，跑本节的「前置字数门 + 8 项语义」校验清单。
...
**0. 字数门（前置机械检查）**：先做这一项，因为字数偏差不需要 LLM 判断，确定性最强。读取本章 `chapter-NNNN.md` 的字符数（含中文标点、不含空白与换行；shell：`tr -d '[:space:]' < chapter.md | wc -m`），比对 `metadata.md` 的 `[word_min, word_max]` 区间（参数计算见本节末尾「字数门参数计算」）：

- 落入区间：通过，进入 1–8 号语义校验
- 偏短（current < word_min）：调用 `enrich_prompt_v2`，输入正文 + `entity_constraints` + `current_length` + `[word_min, word_max]`
- 偏长（current > word_max）：调用 `condense_prompt_v2`，输入正文 + `current_length` + `[word_min, word_max]`
- 严重偏离（缺口或溢出 > 目标的 30%）：直接重写（回到第三步 `next_chapter_draft_prompt_v2`，在 `user_guidance` 槽位追加「上一稿字符数 N，超出区间 X，请严格落入 [word_min, word_max]」）
- 扩/缩写后重新计数；最多 2 次扩缩，仍不通过则走重写
- 1–8 号语义校验仍要执行：字数偏差与语义问题可能并存，避免「先扩写再发现违反硬约束」的二次返工
...
派生：
- `word_min = floor(target × (1 − tol))`
- `word_max = ceil(target × (1 + tol))`

`{word_number} / {word_min} / {word_max}` 这三个槽位在 `first_chapter_draft_prompt_v2` / `next_chapter_draft_prompt_v2` / `enrich_prompt_v2` / `condense_prompt_v2` / `consistency-check-prompt` 中复用同一组值。一个项目的容差只在 `metadata.md` 改一处。
...
**机械校验先行**（有 shell 权限时在语义校验之前运行）：
- `lnw check-naming <N>` — grep 禁用变体，有输出即报错
- `lnw check-progression <N>` — 境界单调性，有输出即报错
...
4. **追加 `canon/facts.jsonl`**：本章新增的约束性事实（含 `known_by` 字段）；ID 分配用 `lnw next-id fact`
5. **追加 `canon/promises.jsonl`**：本章新增或状态变化的承诺；ID 用 `lnw next-id promise`
6. **追加 `canon/progression.jsonl`**：本章发生的境界/能力进阶；ID 用 `lnw next-id prog`
...
`tools/lnw` 需要 shell 执行权限（`execute_bash` 或等价能力）和 `jq`。

| 环境 | shell 权限 | 建议 |
|---|---|---|
| Kiro / Claude Code / Codex | ✅ | 直接调用 `lnw`，全自动 |
| OpenClaw（视配置） | ⚠️ | 确认是否有 execute_bash；无则退化手工 |
| 纯对话框环境 | ❌ | 按本协议手工操作，工具命令仅作参考 |

退化手工时，`lnw` 命令的等价操作：
- `next-id fact` → 读 `canon/facts.jsonl` 最后一行的 id 字段，数字 +1
- `check-naming` → 手工 grep 正文中 `naming.md` 禁用变体列的词
- `check-progression` → 手工比对 `canon/progression.jsonl` 中同实体同系统的历史最高 value
- `snapshot` → 手工 `cp -r entities canon summaries dialogue-samples snapshots/ch-NNNN/`
