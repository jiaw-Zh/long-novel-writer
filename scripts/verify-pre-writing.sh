...
[ "$vol_found" -eq 0 ] && echo "  ⚠ 无 volume 摘要（降级到 story-bible）" && ((warnings++))
for f in "$NOVEL_DIR"/arcs/arc-*.md; do
  [ -f "$f" ] && arc_found=1 && echo "  ✓ $(basename "$f") 存在"
done
[ "$arc_found" -eq 0 ] && echo "  ⚠ 无 arc 摘要（降级到 story-bible）" && ((warnings++))
...
if [ -f "$NOVEL_DIR/blueprints/chapter-${CHAPTER_PAD}.md" ]; then
  ...
  echo "  ✗ chapter-${CHAPTER_PAD} blueprint 缺失" && ((errors++))
fi
next_pad=$(printf "%04d" $((CHAPTER_NUM + 1)))
if [ -f "$NOVEL_DIR/blueprints/chapter-${next_pad}.md" ]; then
  echo "  ✓ chapter-${next_pad} blueprint 存在"
else
  echo "  ⚠ chapter-${next_pad} blueprint 缺失（可选）" && ((warnings++))
fi

# Step 4: Recent 3-5 chapter L1 briefs
echo "[Step 4] 最近 3-5 章 L1 briefs"
l1_count=0
for i in $(seq $((CHAPTER_NUM > 5 ? CHAPTER_NUM - 5 : 1)) $((CHAPTER_NUM - 1))); do
  pad=$(printf "%04d" "$i")
  if [ -f "$NOVEL_DIR/summaries/chapter-${pad}.brief.md" ]; then
    ((l1_count++))
  fi
done
if [ "$l1_count" -ge 3 ]; then
  ...
  echo "  ✗ L1 briefs 不足（找到 $l1_count，需要至少 3）" && ((errors++))
fi
...
if [ -d "$entity_dir" ]; then
  ...
  if [ "$entity_count" -gt 0 ]; then
    ...
    echo "  ⚠ 实体档案目录为空" && ((warnings++))
  fi
else
  echo "  ⚠ 实体档案目录不存在" && ((warnings++))
fi
...
if [ -f "$NOVEL_DIR/foreshadowing-ledger.md" ]; then
  ...
  echo "  ⚠ foreshadowing-ledger.md 不存在" && ((warnings++))
fi
...
if [ -f "$NOVEL_DIR/continuity-issues.md" ]; then
  ...
  echo "  ⚠ continuity-issues.md 不存在（可新建）" && ((warnings++))
fi
if [ -f "$NOVEL_DIR/subplots.md" ]; then
  echo "  ✓ subplots.md 存在"
else
  echo "  ⚠ subplots.md 不存在（可新建）" && ((warnings++))
fi

# Step 9: Keyword reverse lookup
echo "[Step 9] 关键词反查索引"
index_count=$(find "$NOVEL_DIR/summaries" -name "chapter-*.index.md" 2>/dev/null | wc -l)
if [ "$index_count" -gt 0 ]; then
  echo "  ✓ 找到 $index_count 个章节索引"
else
  echo "  ⚠ 无章节索引（首次写入时正常）" && ((warnings++))
fi
